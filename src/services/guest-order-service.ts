 ...
 *
 * The DTO still carries a `turnstileToken` field server-side, but
 * Turnstile has been removed from this flow, so nothing is sent for it
 * and it arrives null. Bot protection for this endpoint is the API rate
 * limiting and IP blocking. If Turnstile is ever reinstated, the field
 * must be added back here AND the verify() call uncommented in the
 * controller — a null token fails verification, so re-enabling only the
 * server side would block every order.
 */
