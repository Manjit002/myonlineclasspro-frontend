 ...
 *
 * ⚠️ `couponCode` is appended only when a coupon has been validated.
 * The backend endpoint must READ this parameter for the discount to be
 * honoured at payment — validating a coupon alone does not apply it.
 * Until then the customer would see a discounted total and be charged
 * the full amount, so this must be confirmed before shipping coupons.
 */
