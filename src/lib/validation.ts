...
export function validatePhone(value: string): string | undefined {
  ...
  if (!trimmed) return ORDER_MESSAGES.phoneRequired;
  // Stored as "<dial> <national>"; validate the national part only, since
  // the dial code's own digits must not count toward the ten.
  const parts = trimmed.split(" ");
  const national = (
    parts.length > 1 ? parts.slice(1).join("") : parts[0]
  ).replace(/\D/g, "");
  if (!national) return ORDER_MESSAGES.phoneRequired;
  if (national.length !== 10) return ORDER_MESSAGES.phoneDigits;
  if (parts.length > 1 && !/^\+\d{1,4}$/.test(parts[0]))
    return ORDER_MESSAGES.phoneRequired;
  return undefined;
}
