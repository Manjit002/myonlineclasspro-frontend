...
/**
 * Transports SockJS is allowed to negotiate.
 *
 * Left unset, SockJS also tries its iframe-based fallbacks
 * (iframe-htmlfile, iframe-eventsource, iframe-xhr-polling) and
 * jsonp-polling. Each of those bootstraps by loading
 * `<sock-url>/iframe.html` as a *document* — i.e.
 * `/ws/chat/iframe.html`. If the server answers that with anything the
 * browser won't render inline, Safari treats it as a file and shows
 * "Do you want to download 'iframe.html'", which is exactly what
 * students hit on iOS right after an order opens the chat.
 *
 * Those transports only exist for browsers without CORS (IE 6-9), so
 * dropping them costs nothing today. WebSocket stays primary and the
 * XHR transports remain as genuine fallbacks, so chat still connects
 * where WebSocket is blocked — the connection path is unchanged, only
 * the negotiated transport list is constrained.
 */
const SOCK_TRANSPORTS = ["websocket", "xhr-streaming", "xhr-polling"];
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const client = new StompClient({
        webSocketFactory: () =>
          new (
            SockJS as unknown as new (
              u: string,
              reserved: null,
              opts: { transports: string[] },
            ) => WebSocket
          )(SOCK_URL, null, { transports: SOCK_TRANSPORTS }),
